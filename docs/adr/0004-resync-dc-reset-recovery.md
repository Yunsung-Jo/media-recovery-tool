...
`0xA1F57000`의 hole 지점에서 측정한 결과 DC 캐리는 clean run best=137(당시 수락 임계 450 미달)이었지만,
**DC를 0으로 리셋하면 best=900**으로 측정 상한에 도달했다. 즉 캐리가 재동기 가능한 지점을 놓치고 있었다.
...
| 재동기 후보를 데이터 소비(skew)로도 채점 | 5개 샘플이 모두 악화됐다. 비트 최소 선호가 우연히 짧게 맞은 잘못된 정렬을 택해 재동기 품질을 떨어뜨렸다. |
...
**향후 고려사항**
- 재동기 세그먼트 Cb/Cr 색 캐스트 보정은 진짜 복구율과 무관한 화질 개선이며
  [현재 상태](../current-state.md)의 후속 과제로 남긴다.
- 비트 과소비나 데이터 소진으로 남는 회색은 재동기 위치 조정만으로 해결할 수 없는 데이터 한계다.
